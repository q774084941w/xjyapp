<?php
require_once "WxTransfers_Config.php";
...
class WxTransfers
{
	...
	public function transfers($parameters){
		...
		if ($resultObj->return_code == 'SUCCESS') { // 成功标识
			...
			if ($resultObj->result_code == 'SUCCESS') {
				...
				$this->partner_trade_no = (string) $resultObj->partner_trade_no;
	...
	{
		$this->_lastPartnerTradeNo = $this->_parameters['mchid'] . date('YmdHis') . $this->getRand(4, 1); // $this->getRandNum();
