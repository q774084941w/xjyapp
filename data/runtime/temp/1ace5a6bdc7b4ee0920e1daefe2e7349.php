<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:49:"themes/admin_simpleboot3/admin\is_order\edit.html";i:1530180016;s:43:"themes/admin_simpleboot3/public\header.html";i:1511766886;}*/ ?>
<!doctype html>
<html>
<head>
    <meta charset="utf-8">
...
<body>
	<div class="wrap js-check-wrap">
		...
		<form action="<?php echo url('admin/isOrder/editPost'); ?>" method="post" class="form-horizontal margin-top-20 js-ajax-form">
        <div class="row">
            <div class="col-md-9">
            	<?php 
            	...
            	<table  class="table table-bordered">  
			        <tr>  
		             	<td style="width: 20%"><font>订单号：</font> <input readonly class="form-control" type="text" name="order[order_id]"
                                    value="<?php echo $order['order_id']; ?>" placeholder="订单号不给于修改" id="order_id" /></td>
			        ...
			        <tr>
			            <td colspan="2"><font>用户评价：</font>
			            	<div>
				            	<textarea  name="order[user_evaluate]" style="width: 100%;height: 100px;resize: none;"><?php if(empty($order['food_evaluate']) || (($order['food_evaluate'] instanceof \think\Collection || $order['food_evaluate'] instanceof \think\Paginator ) && $order['food_evaluate']->isEmpty())): ?>该用户尚未评价！<?php else: ?><?php echo $order['food_evaluate']; endif; ?></textarea>
			            	</div>
			            </td>
						<td colspan="2"><font>备注：</font>
			            	<div>
				            	<textarea  name="order[remarks]" style="width: 100%;height: 100px;resize: none;"><?php if(empty($order['remarks']) || (($order['remarks'] instanceof \think\Collection || $order['remarks'] instanceof \think\Paginator ) && $order['remarks']->isEmpty())): ?>该订单没有备注！<?php else: ?><?php echo $order['remarks']; endif; ?></textarea>
			            	</div>
			            </td>
			        </tr>
			    </table>
							...
							<tbody id="food_this_have">
								<?php if(!(empty($food) || (($food instanceof \think\Collection || $food instanceof \think\Paginator ) && $food->isEmpty()))): if(is_array($food) || $food instanceof \think\Collection || $food instanceof \think\Paginator): if( count($food)==0 ) : echo "" ;else: foreach($food as $key=>$vo): ?>
										<tr>
											...
											<td>
												<input  type="button" class="btn btn-default" value="删除" onclick="res(this)" food="<?php echo $vo['id']; ?>">
											</td>
										</tr>
	...
	<script type="text/javascript" src="__STATIC__/js/bootbox/bootbox.min.js"></script>
		...
		<script>
            ...
            function canDelete(obj)
			{
				...
				$.post("<?php echo url('admin/IsOrder/canDelete'); ?>",
            ...
            function  deleteAjax() {
                var order_id = $('#order_id').val();
                $.post(
                    "<?php echo url('admin/IsOrder/closeRest'); ?>",
                    {'order_id':order_id},
                    function (msg) {
						if(msg.code==1111)
						{
                            $(location).prop('href', "<?php echo url('admin/cashier/index'); ?>")
						}
						else
						{
                            bootbox.setLocale("zh_CN");
