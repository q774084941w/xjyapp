<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:53:"themes/admin_simpleboot3/admin\printer\add_index.html";i:1529732928;s:43:"themes/admin_simpleboot3/public\header.html";i:1511766886;}*/ ?>
<!doctype html>
<html>
<head>
    <meta charset="utf-8">
...
<body>
	<div class="wrap">
		<ul class="nav nav-tabs">
			...
			<li class="active"><a href="<?php echo url('printer/add_index'); ?>">添加设备</a></li>
		</ul>
		<?php 
			$voice = array('无','滴滴声','真人语音(小)','真人语音(中)','真人语音(大)');
		 ?>
		<form method="post" class="form-horizontal margin-top-20 js-ajax-form" action="<?php echo url('printer/addPost'); ?>">
				...
				<div class="col-md-6 col-sm-10" >
					<select class="form-control selectpicker" name="position[]" data-live-search="true" multiple data-actions-box="true">
						<?php if(!(empty($position) || (($position instanceof \think\Collection || $position instanceof \think\Paginator ) && $position->isEmpty()))): if(is_array($position) || $position instanceof \think\Collection || $position instanceof \think\Paginator): if( count($position)==0 ) : echo "" ;else: foreach($position as $key=>$vo): ?>
								<option value="<?php echo $vo['posit_id']; ?>"><?php echo $vo['name']; ?></option>
			...
			<div class="form-group">
				...
				<div class="col-md-6 col-sm-10">
					<input type="text" class="form-control" id="input-name" name="printer_id" placeholder="请输入正确的设备编号">
				</div>
			</div>
			...
			<div class="form-group">
				...
				<div class="col-md-6 col-sm-10">
					<input type="text" class="form-control" id="input-key" name="secret_key" placeholder="请输入正确的设备秘钥">
				</div>
			</div>
			...
			<div class="form-group">
				...
				<div class="col-md-6 col-sm-10" id="input-voice">
					<select class="form-control" name="voice">
						<?php if(is_array($voice) || $voice instanceof \think\Collection || $voice instanceof \think\Paginator): if( count($voice)==0 ) : echo "" ;else: foreach($voice as $key=>$vo): ?>
