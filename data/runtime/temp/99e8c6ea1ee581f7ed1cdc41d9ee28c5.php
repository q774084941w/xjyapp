<?php if (!defined('THINK_PATH')) exit(); /*a:4:{s:49:"themes/wechat_ordering/user\index\scan_order.html";i:1528873488;s:39:"themes/wechat_ordering/public\head.html";i:1528451063;s:38:"themes/wechat_ordering/public\nav.html";i:1524642742;s:41:"themes/wechat_ordering/public\footer.html";i:1528860216;}*/ ?>
<!DOCTYPE html>
<html>
  <head>
    <title>菜单-菜系</title>
  ...
  <body class="grade-a">
    ...
    <div class="weui-search-bar" id="searchBar">
      <form class="weui-search-bar__form" action="<?php echo url('user/index/FoodList'); ?>">
        <div class="weui-search-bar__box">
          ...
          <input type="search" class="weui-search-bar__input" id="searchInput" placeholder="搜索" required="" name="keywords">
    ...
    <div class="weui-tab swiper-container swiper-container-xnav" style="z-index: 88;background-color:#fafafa;border-bottom:1px solid #E4E4E4;">
      <div class="weui-navbar food_list_nav_slide swiper-wrapper">
        <?php if(!(empty($foodClassdata) || (($foodClassdata instanceof \think\Collection || $foodClassdata instanceof \think\Paginator ) && $foodClassdata->isEmpty()))): 
            $parent_id = empty($parent_id) ?  $Request.param.parent_id : $parent_id;
           if(is_array($foodClassdata) || $foodClassdata instanceof \think\Collection || $foodClassdata instanceof \think\Paginator): if( count($foodClassdata)==0 ) : echo "" ;else: foreach($foodClassdata as $key=>$vo): ?>
            <div class="swiper-slide" data-id="<?php echo $vo['id']; ?>">
              <a class="weui-navbar__item  <?php if($vo['id']  == $parent_id): ?> weui-bar__item--on <?php endif; ?>" href="<?php echo cmf_url('user/index/scan_order',['parent_id'=>$vo['id'],'id'=>$table_id['id']]); ?>">
                <?php echo $vo['name']; ?>
              </a>
            </div>
      ...
      <scroll-view scroll-y style="display: block;overflow: auto;" bindscrolltoupper="upper" bindscrolltolower="lower" bindscroll="scroll" id="nav">
            <?php if(!(empty($foodChildClass) || (($foodChildClass instanceof \think\Collection || $foodChildClass instanceof \think\Paginator ) && $foodChildClass->isEmpty()))): if(is_array($foodChildClass) || $foodChildClass instanceof \think\Collection || $foodChildClass instanceof \think\Paginator): if( count($foodChildClass)==0 ) : echo "" ;else: foreach($foodChildClass as $key=>$vo): ?>
                <view id="menuid-" data-idx='' data-foodclass='<?php echo $vo['name']; ?>' data-foodinx='' class="scroll-view-item <?php if($vo['id']  == \think\Request::instance()->param('child_id')): ?> scroll-view-item-activ <?php endif; ?>" bindtap="filter" >
                  <!-- <a href="<?php echo cmf_url('user/index/scan_order',['parent_id'=>$vo['parent_id'],'child_id'=>$vo['id'],'id'=>$table_id['id']]); ?>"> <?php echo $vo['name']; ?> </a> -->
    ...
    <view class="footer-shopping-Selected-products" hidden="" style="display: none;overflow-y: scroll;height:300px;">
      ...
      <div class='booking-select' style="border-top:1px solid #ECECEC;margin-bottom: 20px">
        <table style="font-size: 20px;width: 100%">
          <tr>
            <th> <?php echo $table_id['name']; ?>厅</th>
            <th><?php echo $table_id['table_name']; ?></th>
            <th>  第 <font color="#A146E1"><?php echo $table_id['tb_id']; ?></font>号桌</th>
          </tr>
        </table>
      </div>
    </view>
    ...
    <view class="footer-shopping-cart-fixed" hidden="">
      <form action="<?php echo cmf_url('user/index/place_order'); ?>" class="js-ajax-form" method="post" onsubmit="return Scavenging()">
        <input type='hidden' value='<?php echo $table_id["id"]; ?>' id="tb_id" name="tb_id" />
